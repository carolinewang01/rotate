...
class PLRBuffer(struct.PyTreeNode):
	...
	coplayer_count: chex.Array # how often coplayer is sampled in MAESTRO
	max_selfplay_return: chex.Array # the coplayer's max self-play return on each level
	max_return_ever: chex.Array # store the highest return ever achieved with that coplayer...
	current_max_return_ever: chex.Array
	ema_max: chex.Array
	returns_window: chex.Array
...
class PLRManager:
	def __init__(
		...
		self.coplayer_count = jnp.zeros((1,), dtype=jnp.int32)
		self.max_selfplay_return = jnp.full(buffer_size, -jnp.inf)
		self.max_return_ever = jnp.zeros((5,), dtype=jnp.float32)
		self.current_max_return_ever = jnp.zeros((1,), dtype=jnp.float32)
		self.ema_max = jnp.zeros((5,), dtype=jnp.float32)
		self.returns_window = jnp.zeros((5,10), dtype=jnp.float32)
	...
	def reset(self):
		return PLRBuffer(
			...
			coplayer_count=self.coplayer_count,
			max_selfplay_return=self.max_selfplay_return,
			max_return_ever=self.max_return_ever,
			current_max_return_ever=self.current_max_return_ever,
	...
	def _get_replay_dist(self, scores, ages, filled):
		...
		if self.use_score_ranks:
			_, unique_inverse = jnp.unique(scores, size=len(scores), fill_value=0, return_index=True)
			ranks = jnp.max(unique_inverse) + 1 - unique_inverse # still works when there's -inf
			scores = 1 / ranks

			# sorted_idx = jnp.argsort(-scores) # Top first
			# scores = jnp.zeros(self.buffer_size, dtype=jnp.float32)\
			# 		.at[sorted_idx]\
			# 		.set(1/jnp.arange(1, 1+self.buffer_size))
	...
	def sample(self, rng, plr_buffer, new_levels, n, random=False):
		"""This will get vmapped over n_agents later.
		
		new_levels: (n_envs, n)
		n: n_envs
		
		Same replay decision for all parallel envs, but different levels may get sampled.
		"""
	...
	def update(self, plr_buffer, levels, level_idxs, ued_scores, dupe_mask=None, info=None, ignore_val=-jnp.inf, parent_idxs=None):
		"""Update a single agent's buffer, scanning over the parallel envs.

		arg shapes:
			plr_buffer: (buffer_size, *)
			levels, level_idxs, ued_scores: (n_envs, *)

		outputs next_plr_buffer of same shape as input plr_buffer.

		This will then get vmapped over n_agents in PopPLRManager.

		I assume level_idxs is -1s if the level is not replayed?
		"""
		...
		# This function will get scanend over num_envs
		def update_level_info(carry, step):
			plr_buffer, insert_mask = carry
			levels = plr_buffer.levels # (buffer_size, *)
			scores = plr_buffer.scores # (buffer_size,)
			filled = plr_buffer.filled

			score, level, level_idx, done_mask, parent_idx, max_return, max_selfplay_return = step # this is *one* level
			# max_return is max crossplay return
			...
			# compare input score with existing score
			...
			should_insert_or_update = jnp.logical_or(should_insert, should_update) # single bool
			...
			next_max_selfplay_return = jnp.where(
				should_insert_or_update,
				plr_buffer.max_selfplay_return.at[insert_idx].set(max_selfplay_return),
				plr_buffer.max_selfplay_return
			) # is this right?
			...
			plr_replace_kwargs = dict(
				...
				max_selfplay_return=next_max_selfplay_return,
			)
		...
		if plr_buffer.ued_score == UEDScore.COPLAYER_REGRET.value:
			max_selfplay_return = info['max_coplayer_returns']
		else:
			max_selfplay_return = jnp.full_like(level_idxs, -1)

		if plr_buffer.ued_score == UEDScore.BOOTSTRAP.value:
			ema_max = info['ema_max']
		else:
			ema_max = jnp.zeros_like(plr_buffer.ema_max)

		# if plr_buffer.ued_score == UEDScore.BOOTSTRAP.value:
		# 	current_max_return_ever = info['current_max_return_ever']
		# else:
		# 	current_max_return_ever = jnp.zeros((1,), dtype=jnp.float32)
		current_max_return_ever = jnp.zeros((1,), dtype=jnp.float32)

		scan_over = (ued_scores, levels, level_idxs, done_masks, parent_idxs, max_returns, max_selfplay_return)

		# Scan over parallel environments
		...
		max_return_ever = info['max_return_ever']
		# ema_max = 0.01*max_return_ever + 0.99*plr_buffer.ema_max

		next_plr_buffer = next_plr_buffer.replace(max_return_ever=max_return_ever, current_max_return_ever=current_max_return_ever, ema_max=ema_max)
