...
def run_heldout_evaluation(config, print_metrics=False):
    ...
    ego_policy, ego_params, init_ego_params = initialize_rl_agent_from_config(ego_agent_config, env, ego_init_rng)
    ego_params_list = extract_params(ego_params, init_ego_params)
...
def print_metrics_table(eval_metrics, metric_name, ego_names, heldout_names):
    ...
    table = PrettyTable()
    table.field_names = [metric_name, *heldout_names]
