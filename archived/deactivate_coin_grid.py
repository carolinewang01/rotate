'''
TODO: we want to modify this environment to make it into a toy gridworld for 
experiments. We will build off this existing environment.

Main modifications:
- State space: an n x n grid with the positions of the agents, the coin, and the amount of time remaining.
- Observation space: each agent should observe the positions of itself, the other agent, the coin, whether the coin is active, and the amount of time remaining.
- Action space: standard movement actions (up, down, left, right, stay), an action to deactivate the coin, and an action to collect the coin.
- Dynamics: 
    - to collect the coin, both agents must be adjacent to the coin and both take the collect action simulatenously. 
    - to deactivate the coin either agent can take the deactivate action.
    - no two agents can be in the same cell at the same time. 
    - if an agent attempts to move into a cell that is occupied by another agent, the wall, or the coin, the agent will stay in its current cell.
- Reward structure: 
    - There is a reward for collecting the coin.
    - There is a per-timestep penalty for not collecting the coin.
- Done: the episode is done when the coin is collected or when the time runs out.
- Initial conditions: 
    - Each agent has a fixed initial position. 
    - There is only one coin with a fixed initial position near the center of the grid. 
    - The amount of time in each episode is configurable but fixed.

Please also modify the rendering function to visualize the new gridworld.
The agents should be visualized as different colors. 
'''
