...
def train_fcp_agent(config, checkpoints):
    ...
    def make_fcp_train(config, partner_params):
        ...
        def train(rng):
            ...
            agent0_net = ActorCriticRNN(env.action_space(env.agents[0]).n)
            ...
            init_hstate_0 = ScannedRNN.initialize_carry(config["NUM_ENVS"])
            ...
            def _env_step(runner_state, unused):
                ...
                # TODO: verify that inputs are the right shape
                # TODO: check that we're passing done through correctly
                rnn_input_0 = (
                    obs_0.reshape(1, config["NUM_ENVS"], -1),  # obs
                    last_done.reshape(1, config["NUM_ENVS"]),  # done
                    avail_actions_0.reshape(1, config["NUM_ENVS"], -1)  # avail_actions
                )

                # Agent_0 action
                pi_0, val_0, new_hstate_0 = agent0_net.apply(train_state.params, hstate_0, rnn_input_0)
                ...
                new_runner_state = (train_state, env_state_next, obs_next, done, new_hstate_0, partner_indices, rng)
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, target_v):
                        ...
                        seq_len = traj_batch.obs.shape[1] # TODO: check how to get seq len!
                        init_h_0 = ScannedRNN.initialize_carry(batch_size)
                        rnn_input_0 = (
                            traj_batch.obs, # .reshape(1, batch_size, -1),
                            traj_batch.done, # .reshape(1, batch_size),
                            traj_batch.avail_actions # .reshape(1, batch_size, -1)
                        )

                        pi, value, _ = agent0_net.apply(params, init_h_0, rnn_input_0) # APPLY FLAG
                ...
                # TODO: check if we need to change the batch sampling to preserve temporal structure. 
                batch = jax.tree.map(
            ...
            def _update_step(update_runner_state, unused):
                ...
                # 1) rollout
                runner_state = (train_state, env_state, last_obs, last_done, hstate_0, partner_indices, rng)
                ...
                rnn_input_0 = ( # TODO: determine if we need to reshape this btach
                    last_obs_batch_0, # .reshape(1, config["NUM_ENVS"], -1),
                    last_done, # .reshape(1, config["NUM_ENVS"]),
                    avail_actions_0 # .reshape(1, config["NUM_ENVS"], -1)
                )

                _, last_val, _ = agent0_net.apply(train_state.params, hstate_0, rnn_input_0) # APPLY FLAG
            ...
            update_steps = 0
            # TODO: check whether init_done should be num envs or num actors
            init_done = jnp.zeros((config["NUM_ENVS"]), dtype=bool)
            update_runner_state = (train_state, env_state, obsv, init_done, init_hstate_0, 
                                   partner_indices, rng, update_steps)
