        ...
        for metric_name in metric_names:
            ...
            try:
                ...
                if is_oel_method:
                    ...
                    mean_returns = mean_data.mean(axis=(0, 1, 3))  # Shape (num_heldout_agents,)
                    ...
                    mean_returns = mean_data.mean(axis=(0, 2))  # Shape (num_heldout_agents,)
...
def compute_best_returns(task_name: str) -> Dict[str, List[float]]:
    ...
    # Initialize dict to store best returns for each heldout agent and each metric
    best_returns = {}
        ...
        try:
            ...
            for metric_name, values in returns.items():
                if metric_name not in best_returns:
                    # First time seeing this metric, initialize the array
                    ...
                    # Compare and update best returns
                    best_returns[metric_name] = np.maximum(best_returns[metric_name], values)
        except Exception as e:
